  ...
  render() {
    ...
    const workflowEditElement = (
      <Row className="mt-2">
        ...
        <Col>
          <MultiSelect ref={this.multiSelectRef} groups={this.state.workflowTemplate.groups}/>
        </Col>
            ...
            this.state.workflowTemplate.triggers.map((trigger, keyTrigger) =>
              <ListGroupItem>
                <ListGroupItemHeading className="d-flex workflow-edit-font-trigger-head">
                  <X size="15" className="x-closer" onClick={() => {
                        ...
                        <DropdownMenu>
                          {
                            this.state.triggerTypes.map((trigger, label) =>
                              <DropdownItem onClick={() => this.setWorkflowTriggerType(keyTrigger, trigger.type)}
                                            tag="button">{trigger.label}</DropdownItem>
                            )
                          }
                        </DropdownMenu>
                      </UncontrolledButtonDropdown>
                      <UncontrolledButtonDropdown>
                        <DropdownToggle disabled={!this.checkTriggersTypeIsCorrect(trigger.trigger_type)}
                                        style={{'border-radius': 0}} color="primary" size="sm" caret>
                        ...
                        <DropdownMenu>
                          {
                            this.getTriggersByType(trigger.trigger_type).map((triggerObj) =>
                              <DropdownItem onClick={() => this.setWorkflowTrigger(keyTrigger, triggerObj)}
                                            tag="button">{triggerObj.trigger}</DropdownItem>
                            )
                          }
                        </DropdownMenu>
                      </UncontrolledButtonDropdown>
                              ...
                              <DropdownMenu>
                                {
                                  this.state.actionTypes.map((action, label) =>
                                    <DropdownItem
                                      onClick={() => this.setWorkflowTriggerActionType(keyTrigger, keyAction, action.type)}
                                      tag="button">{action.label}</DropdownItem>
                                  )
                                }
                              </DropdownMenu>
                            </UncontrolledButtonDropdown>
                            <UncontrolledButtonDropdown>
                              <DropdownToggle disabled={!this.checkTriggersActionTypeIsCorrect(action.action_type)}
                                              style={{'border-radius': 0}} color="primary" size="sm" caret>
                              ...
                              <DropdownMenu>
                                {
                                  this.getTriggerActionsByType(action.action_type).map((actionObj) =>
                                    <DropdownItem
                                      onClick={() => this.setWorkflowTriggerAction(keyTrigger, keyAction, actionObj)}
                                      tag="button">{actionObj.action || actionObj.name}</DropdownItem>
                                  )
                                }
                              </DropdownMenu>
                            </UncontrolledButtonDropdown>
                              ...
                              <div className="text-center w-100">
                                <div className="text-center w-100">
                                  ...
                                  <CreatableSelect
                                    ...
                                    onChange={(event) => {
                                      this.changeUserTypeOption(event, keyTrigger, keyAction)
                                    }}
                                  />
                                </div>
                                {
                                  action.user_target_type === this.userTargetTypes.managers ?
                                    <div className="text-center w-100 mt-2">
                                      ...
                                      <Select
                                        ...
                                        className="fix-margin-select"
                                        onChange={(values) => {
                                          this.onChangeActionUser(values, keyTrigger, keyAction)
                                        }}
                                        classNamePrefix="select"
                                      />
                                    </div>
                ...
                <div className="d-flex justify-content-end flex-wrap mt-2">
                  <Button size="sm" color="primary d-flex-left" onClick={() => this.createAction(keyTrigger)}>CREATE
                    ACTION</Button>
                </div>
              </ListGroupItem>
            )
            ...
            </Col>
            {
              this.state.workflowEdit ?
                <Col col="md-6">
                  <Card>
                    <CardHeader>
                      <CardTitle className="font-weight-bold">{this.state.workflowTemplate.name} <Badge
                        color="info">{this.state.workflowModalType}</Badge></CardTitle>
