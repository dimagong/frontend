...
import { Nav, NavItem, TabContent, Row, Col, NavLink, TabPane, Button, CardTitle, Card, FormGroup, CardBody, CardHeader } from 'reactstrap';
...
class FormCreate extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            isSubmitHidden: props.isSubmitHidden === true ? true : false,
            dFormActions: [
        ...
        //this.state.uiSchema.
        this.state.type = this.state.controlTypes[0];
            ...
            return Object.keys(groupedElements).map((groupName, index) => {
                ...
                const elementContent = groupedElements[groupName].map(element => {
                    if (isElementInSection(element.content.key, sectionName)) {
                        ...
                        return (<div style={isElementHidden(element.content.key)} className={getColumnClass(element.content.key)} key={element.content.key}>
                            {element.content}
                        </div>)
                    }
                    return null;
    ...
    formSubmit = (event) => {
        ...
        if (this.props.submitOnboardingForm) {
            this.props.submitOnboardingForm(formData);
        }
    ...
    getListControls(properties) {
        ...
        const renderConfigFields = ((objKey, index) => {
                        ...
                        return (<div>
                            <div className="row" key={index}>
                                <div className="col-md-12 form-group">
                                    ...
                                    <select id="uischema-multiselect-checkboxes" className="form-control" value={this.getUiSchemaTemplateMultiselect(objKey)} onChange={(event) => this.changeUiSchemaTemplateMultiselect(event)}>
            ...
            return (
                <div className={objKey in this.state.uiSchema.columnsClasses ? this.state.uiSchema.columnsClasses[objKey] : 'col-md-12'} key={'control-edit-' + objKey + '-' + index}>
                    ...
                    <div className="d-flex dform-input-setting">
                        <div className="vertical-center dform-input">
                            <ElementEditModal onOpen={() => this.elementEditModalOpened(objKey)} onSave={() => this.elementEditModalSave(objKey)}>
                                {renderLabel('property-' + objKey, 'Property')}
                        ...
                        <div className="vertical-center dform-input">
                            <DependencyEditModal onOpen={this.uiSettingsOpen.bind(this, objKey)} onSave={this.uiSettingsSave.bind(this, 'fields', objKey)} onDelete={() => { this.removePropertyField(objKey) }}>
                                <div className="">
                                    <div className="col-md-12 form-group">
                                        <label>Section</label>
                                        <select
                    ...
                    <Card className="mt-1" key={index}>
                        ...
                        <CardBody>
                                        ...
                                        <label>Sections</label>
                                        {
                                            condition.sections.map((condition, indexField) => {
                                                return (<div className="row ml-2 mt-1" key={index + '_' + indexField}>
                                                    ...
                                                    <div className="vertical-center dform-input">
                                                        <X size={15} className="cursor-pointer mr-1" onClick={event => this.removeConditionalField(event, 'sections', index, indexField)} />
                                                    </div>
                                                </div>)
                                            })
                                        }
                                        <div className="row m-2">
                                            <div className="form-create__add-new-group small" onClick={this.addConditionField.bind(this, 'sections', index)}   >
                                                Add section
                                    </div>
                                        </div>
                                    </div>
...

                                {
                                    condition.groups.map((condition, indexField) => {
                                        return (<div className="row mt-1 ml-2" key={indexField}>
                                            ...
                                            <div className="vertical-center dform-input">
                                                <X size={15} className="cursor-pointer mr-1" onClick={event => this.removeConditionalField(event, 'groups', index, indexField)} />
                                            </div>
                                        </div>)
                                    })
                                }
                                <div className="row m-2">
                                    <div className="form-create__add-new-group small" onClick={this.addConditionField.bind(this, 'groups', index)}   >
                                        Add group
                                    </div>
                                </div>
                            </div>
                                    ...
                                    condition.fields.map((condition, indexField) => {
                                        return (<div className="row mt-1 ml-2" key={indexField}>
                                            <div className="col-md-10">
                                                ...
                                                >
                                                    <option key="-1"></option>
                                                    {Object.keys(this.state.schema.properties).filter(nextFilterField => nextFilterField !== objKey).map((type, indexType) => <option
                                                        key={indexType}>{type}</option>)}
                                                </select>
                                            </div>

                                            <div className="vertical-center dform-input">
                                                <X size={15} className="cursor-pointer mr-1" onClick={event => this.removeConditionalField(event, 'fields', index, indexField)} />
                                            </div>
                                        </div>)
                                    })
                                }
                                <div className="row m-2">
                                    <div className="form-create__add-new-group small" onClick={this.addConditionField.bind(this, 'fields', index)}   >
                                        Add field
                                    </div>
                                </div>
                            </div>
                            <div>
                                ...
                                {condition.fieldOperators.length ?
                                    ...
                                    condition.fieldOperators.map((fieldOperator, indexFieldOperator) => {
                                        return (<div className="row mt-1 ml-2" key={indexFieldOperator}>
                                            ...
                                            <div className="vertical-center dform-input">
                                                <X size={15} className="cursor-pointer mr-1" onClick={event => this.removeConditionalField(event, 'fieldOperators', index, indexFieldOperator)} />
                                            </div>
                                        </div>)
                                    })
                                    : null
                                ...
                                <div className="row m-2">
                                    <div className="form-create__add-new-group small" onClick={this.addOperatorField.bind(this, 'fieldOperators', index)}  >
                                        Add operator
                                    </div>
                                </div>
                            </div>
    ...
    modalEditDependencies = (dependencyType, objKey) => {
        ...
        return (
            <DependencyEditModal onOpen={() => this.dependencyModalOpen(dependencyType, objKey)} onSave={() => this.dependecyModalSave(dependencyType, objKey)} onDelete={() => { this.deleteGroupOrSection(dependencyType, objKey) }}>
                ...
                <div className="border-top">
                    ...
                    <div className="row m-2">
                        ...
                        <div className="form-create__add-new-group" onClick={this.addConditional.bind(this, dependencyType, objKey)} >
                            Add condition
                        </div>
                    </div>
                </div>
            ...
            <Row>
                {
                    this.props.isStateConfig ?
                        <Col>
                            ...
                            <div className="form-group border-bottom">
                                <label>Name</label>
                                <input value={this.state.additionalData.name} onChange={(event) => { this.setState({ additionalData: { ...this.state.additionalData, name: event.target.value } }) }} type="text"
                                    className="form-control" />
                            </div>
                            <div className="form-group border-bottom">
                                <label>Description</label>
                                <input value={this.state.additionalData.description} onChange={(event) => { this.setState({ additionalData: { ...this.state.additionalData, description: event.target.value } }) }} type="text"
                                    className="form-control" />
                            </div>
                            <div className="">
                                {controls}
                        ...
                        <Col>
                            ...
                            >
                                <div className="form-create__dform_actions pr-1">
                                    {
                                        this.state.isSubmitHidden ? null : <Button type="submit" color="primary">Save</Button>
                                    }
                                    {
                                        this.props.statusChanged ?
                                            ...
                                            <div style={{ width: '160px' }}>
                                                <Select
                                                    ...
                                                    onChange={(value) => { this.setSelectedDFormAction(value) }}
                                                />
                                            </div>
                                            : null
                                    }
                                    {
                                        this.props.onSubmit ? <Button type="button" color="primary" onClick={() => {this.onSubmit()}}>Submit</Button> : null
                                    }
                                </div>
                            </Form>
