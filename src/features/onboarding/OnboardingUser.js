...
import { submitdFormRequest, submitdFormDataRequest, setProfileOnboarding } from "app/slices/appSlice";
...
const OnboardingUser = () => {
  ...
  const isOnboarding = () => {
    return profile && profile.user && profile.onboardings && profile.onboardings.length
  }
...
const submitOnboardingForm = data => {
  dispatch(submitdFormDataRequest({dForm: profile.onboarding.d_form, data}))
}
