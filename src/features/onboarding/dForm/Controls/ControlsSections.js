...
const ControlsSections = ({ sectionName }) => {
  ...
  return sectionGroupsKeys.map((groupName, index) => {
    ...
    if (groupedElementsKeys.indexOf(groupName) !== -1) {
      ...
      const elementContent = groupedElementKeys.map((key) => {
    ...
    if (schema.uiSchema.sectionGroups[groupName] !== sectionName) return null;
