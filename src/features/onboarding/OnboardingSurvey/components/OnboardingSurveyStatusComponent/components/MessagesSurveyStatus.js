...
const MessagesSurveyStatus = ({ status, isAllApplicationsCompleted, isFeedbackExist, is_show_result, children }) => {
  return (
    <div className={`status ${status}`}>
      ...
      <div className={"status_description"}>
        ...
        {(isFeedbackExist || is_show_result) && <div className={"status_description_action"}>{children}</div>}
      </div>
    </div>
  );
};

export default MessagesSurveyStatus;
