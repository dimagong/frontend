...
const { updateUserAvatarRequest, deleteUserAvatarRequest, getUserAvatarRequest } = appSlice.actions;
...
const UserAvatar = () => {
  ...
  return (
    <Media left className="user-edit__user-avatar mt-md-1 mt-0 mr-1">
      ...
      <div className="user-edit__user-avatar_button-wrapper">
        <Button disabled={isLoading} onClick={(event) => changeAvatar(event)} outline size="sm" color="primary">
          Change
        </Button>
        <input
      ...
      {manager.url && !isLoading ? <X className="x-closer" onClick={removeAvatar} size={15} /> : null}
      {isLoading ? (
