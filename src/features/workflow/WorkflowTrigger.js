...
import { selectWorkflow, selectdFormTriggers } from "app/selectors/onboardingSelectors";
...
const WorkflowTrigger = ({ keyTrigger, trigger, context }) => {
  ...
  const getTriggerNameById = (id, type) => {
    ...
    if (type === "survey_trigger") {
      trigger =
        type === triggerByTriggerType[type].trigger ? surveyTriggers.find((trigger) => trigger.id === id) : null;
    } else {
      trigger = type === triggerByTriggerType[type].trigger ? triggers.find((trigger) => trigger.id === id) : null;
    }
  ...
  const removeTrigger = () => {
    if (window.confirm("Are you sure?")) {
      dispatch(setWorkflowTriggers(workflow.triggers.filter((element) => element.id !== trigger.id)));
    }
  };
  ...
  const setTriggerProperty = (triggerProperty) => {
    dispatch(
      setWorkflowTriggers(
        workflow.triggers.map((element) => (element.id === trigger.id ? { ...element, ...triggerProperty } : element))
      )
    );
  };
  ...
  const getListTriggers = () => {
    ...
    if (
      ...
      return triggers.map((trigger) => (
        <DropdownItem onClick={() => setTriggerProperty({ trigger_id: trigger.id })} tag="button">
          {trigger.trigger}
        </DropdownItem>
      ));
    } else if (
    ...
    <ListGroupItem>
        ...
        <div className="d-flex flex-wrap justify-content-center align-items-center w-100">
          ...
          <div className="text-center w-100">
            ...
            <UncontrolledButtonDropdown>
              ...
              <DropdownMenu>{getListTriggers()}</DropdownMenu>
            </UncontrolledButtonDropdown>
          </div>
        </div>
      </ListGroupItemHeading>
      <ListGroup className="d-flex mt-2">
        {trigger.actions.map((action, keyAction) => (
          <WorkflowAction action={action} keyAction={keyAction} keyTrigger={keyTrigger} trigger={trigger} />
        ))}
      </ListGroup>
