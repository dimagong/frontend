...
const SortingFilters = ({ currSort, setCurrSort, applyFilters }) => {
  ...
  const handleSortingBox = () => {
    let sortingBox = document.getElementsByClassName('sorting-box');
    if (sortingBox[0].style.display === "none") {
      sortingBox[0].style.display = "block";
    } else {
      sortingBox[0].style.display = "none";
    }
  }
  ...
  return <span onClick={handleSortingBox} className={'sorting-filter'}>
