...
const SortingFilters = ({ currSort, setCurrSort, applyFilters }) => {
  ...
  return (
    <span onClick={() => setIsSortingBoxOpen(!isSortingBoxOpen)} className={"sorting-filter"}>
      ...
      <span className={"sorting-box " + (isSortingBoxOpen ? " opened" : " closed")}>
        {elements.map((item) => (
          <div
            key={item.id}
            onClick={() => handleSwitchSort(item.id)}
            className={"sorting-filter-box " + (currSort === item.id ? "active" : "")}
          >
            {item.data}
          </div>
        ))}
