...
const FilterOptions = ({ filter, curr, roles, memberFirms, organizations, handleFilterOptions }) => {
  ...
  switch (curr) {
    case "roles":
      ...
      break;
    case "organizations":
      currFilters = organizations;
      break;
    case "memberFirms":
      currFilters = memberFirms;
      break;
    case "reps":
      currFilters = [];
      /*TODO*/ break;
  ...
  return currFilters.map((item, idx) => (
    <Button className={"filter-option not-active"} variant="primary" key={idx}>
      ...
      <span className={"filter-right"}>
        <span>
          <span className={"filter-check"} onClick={() => handleFilterOptions("remove", item)}>
            <svg
              style={
                filter.type[curr] === "cross" && !filter[curr].find((el) => el === item)
            ...
            >
              <path
                ...
                fillRule="evenodd"
                className="shp0"
                d="M9.23 0.87C9.93 0.87 10.51 1.45 10.51 2.15L10.51 9.85C10.51 10.55 9.93 11.13 9.23 11.13L1.53 11.13C0.83 11.13 0.25 10.55 0.25 9.85L0.25 2.15C0.25 1.45 0.83 0.87 1.53 0.87L9.23 0.87ZM7.34 9.07L7.34 9.07L7.34 9.07L7.34 9.07ZM7.34 9.07L8.45 7.96L6.49 6L8.45 4.04L7.34 2.93L5.38 4.89L3.42 2.93L2.31 4.04L4.27 6L2.31 7.96L3.42 9.07L5.38 7.11L7.34 9.07Z"
              />
            </svg>
          </span>
          <span className={"filter-check"} onClick={() => handleFilterOptions("add", item)}>
            <svg
              style={
                filter.type[curr] === "check" && filter[curr].find((el) => el === item)
