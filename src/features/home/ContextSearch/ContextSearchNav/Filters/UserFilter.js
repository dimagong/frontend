...
const UserFilter = ({ handleFilter, managers }) => {
  ...
  organizationsObjects.forEach((item) => {
    organizations.push(item.name.replace("_", " "));
  });
  ...
  const wrapperRefFilterBox = useRef(null),
    wrapperRefFilterButton = useRef(null);
  useOutsideAlerter([wrapperRefFilterBox, wrapperRefFilterButton], () => {
    if (!isDeleteModalOpen) setIsFilterBoxOpened(false);
  });
  ...
  const handleFilterOptions = (type, option) => {
    ...
    setFilter(newFilter);
    setFooterText({
      roles: arrayToString(newFilter.roles),
      organizations: arrayToString(newFilter.organizations),
      memberFirms: arrayToString(newFilter.memberFirms),
    });
  };
  ...
  const changeFooterText = (filter) => {
    setFooterText({
      roles: arrayToString(filter.roles),
      organizations: arrayToString(filter.organizations),
      memberFirms: arrayToString(filter.memberFirms),
    });
  };
  ...
  const footer = () => {
    if (
      (filter.roles.length === 0 && filter.organizations.length === 0) ||
    ...
    filterAndSortManagers(
      {
        ...
        memberFirms: appliedFilters.memberFirms,
      },
      appliedFilters.sort
    );
  ...
  useEffect(() => {
    ...
    if (filter.memberFirms.length !== newMemberFirms.length) {
      let newFilter = {
        ...
        memberFirms: [...filter.memberFirms],
        type: {
  ...
  return (
    <span className={"filters"}>
      ...
      {filter.roles.length !== 0 && (
        <Button className={"filter-tab"} variant={"dark"}>
          <span className={"nav-text"}>
            {footerText.roles.length <= MAX_TABS_LENGTH ? footerText.roles : `${filter.roles.length} roles`}
          </span>
          <span
            onClick={() =>
              handleCloseTab({
          ...
          <span
            onClick={() =>
              handleCloseTab({
                ...
                organizations: [],
                memberFirms: filter.memberFirms,
                type: { roles: filter.type.roles, organizations: "initial", memberFirms: filter.type.memberFirms },
              })
            }
            className={"close-nav"}
          >
      ...
      {filter?.memberFirms?.length !== 0 && (
        <Button className={"filter-tab"} variant={"dark"}>
          <span className={"nav-text"}>
            {footerText.memberFirms.length <= MAX_TABS_LENGTH
              ? footerText.memberFirms
              : `${filter.memberFirms.length} member firms`}
          </span>
          <span
            onClick={() =>
              handleCloseTab({
                ...
                organizations: filter.organizations,
                memberFirms: [],
                type: { roles: filter.type.roles, organizations: filter.type.organizations, memberFirms: "initial" },
              })
            }
            className={"close-nav"}
          >
