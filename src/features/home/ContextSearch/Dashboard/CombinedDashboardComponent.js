...
import { selectDashboardDataByKey, selectDashboardDForms, selectManagers } from "app/selectors/userSelectors";
...
const { getDashboardDataRequest, getDashboardActivityRequest, getDashboardSnapshotDataRequest } = appSlice.actions;

const CombinedDashboardComponent = ({
  chartId,
  chartType,
  dashboardSettings,
  updateSettings,
  ...
  return (
    <div
      className={`combined-dashboard-component ${
        isFilterBoxOpen ? "combined-dashboard-component-filtered" : ""
      } combined-dashboard-component-${settings.state === "small" ? "small" : "large"}`}
    >
      <div
        className={"dashboard-charts"}
        style={
          settings.state === "large"
            ? { backgroundImage: "linear-gradient(to bottom, rgba(0,0,0,0) 20%, white 20%)" }
            : {}
        }
      >
        <div style={{ width: "100%" }} className={"dashboard-one-chart"}>
          <span className={"arrow-left"}>
            <img src={settings.state !== "small" ? ArrowLeft : ArrowRight} alt={"arrow"} onClick={handleChangeChart} />
            <img
              src={CloseChart}
          ...
          <span className={"change-chart-days " + (settings.state === "small" ? " change-chart-days-smaller" : "")}>
            {chartType === "Applications" && settings.dForm !== "Unselected application" && settings.dForm?.id && (
              <span>
                ...
                <span
                  ...
                  onClick={() => setIsMapFilterBoxOpen(!isMapFilterBoxOpen)}
            ...
            {settings?.dForm?.name !== "Applications Snapshot" &&
              [
                { label: "y", daysNumber: 365 },
                { label: "m", daysNumber: 28 },
                { label: "w", daysNumber: 7 },
              ].map((item, idx) => {
                return (
                  <span
          ...
          <LineChart
            ...
            data={
              chartType === "Activities"
                ? dashboardData?.usersActivitiesSchedule
                : dashboardData?.userDFormActivitiesSchedule
            }
          />
        </div>
      </div>
      ...
      <div
        style={{ background: "white", zIndex: 100 - settings.key }}
        className={
          "dashboard-activities " +
          (settings.state === "large" ? "dashboard-activities-open" : "dashboard-activities-closed")
        }
      >
        <ActivitiesDashboard
          usersActivities={
            chartType === "Applications" && !settings.dForm?.id
              ? []
              : chartType === "Applications"
              ? dashboardData?.userDFormActivities
              : dashboardData?.usersActivities
          }
          settings={settings}
