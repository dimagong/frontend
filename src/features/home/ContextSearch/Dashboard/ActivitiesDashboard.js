...
const ActivitiesDashboard = ({
  ...
  useOutsideAlerter([wrapperRefFilterBox, wrapperRefFilterButton], () => {
    if (isFilterBoxOpen) setIsFilterBoxOpen(false);
  });
    ...
    if (settings.title === "Activities") {
      if (settings?.dForm?.name === "Applications Snapshot") {
        dispatch(
          getDashboardSnapshotDataRequest({
            key: settings.key,
            page: usersActivities.current_page + 1,
  ...
  const handleActionClick = (manager, currAction) => {
    ...
    let newManager = selectedInfo ? { selectedInfo: selectedInfo, ...manager } : { ...manager };
  ...
  const FilterIconAndTabsBox = () => {
    return (
      <span>
        ...
        {settings.filter &&
          Object.keys(settings.filter).map((key) => {
            if (
              (key !== "Application" || settings?.dForm?.name === "Applications Snapshot") &&
              Array.isArray(settings.filter[key]) &&
              settings.filter[key].length > 0
            ) {
              return (
                <Button style={{ zIndex: 1000000 }} className={"filter-tab"} variant={"dark"} key={key}>
                  {key === "Application" ? (
                    <span className={"nav-text"}>
                      {settings.filter[key].length}{" "}
                      {settings.filter[key].length > 1 ? key.toLowerCase() + "s" : key.toLowerCase()}
                    </span>
                  ) : (
                    <span className={"nav-text"}>
                      {settings.filter[key].length}{" "}
                      {settings.filter[key].length > 1 ? key.toLowerCase() : key.toLowerCase().slice(0, -1)}
                    </span>
                  )}
                  <span onClick={() => removeFilterPart(key)} className={"close-nav"}>
                    <CloseIcon />
                  </span>
                </Button>
              );
            ...
            activities.map((item, key) => (
              <div style={{ paddingLeft: "5px" }} key={key}>
                <div className={"action-date"} style={{ position: "relative" }}>
                  ...
                  {key === getFirstCheckedDate() && <FilterIconAndTabsBox />}
                </div>
                {/* eslint-disable-next-line array-callback-return */}
                {item.data.map((currAction, idx) => {
                  if (
                    !currAction.options.hasOwnProperty("show_in_dashboard") ||
                    !currAction.options.show_in_dashboard
                  ) {
                    // eslint-disable-next-line array-callback-return
                    return;
                  }
                  ...
                  let description =
                    activityTypes.find((item) => item.id === currAction.action_type_id)?.name === userProfileUpdated
                      ? getEditMessage(currAction)
                      : parseTextToComponent(currAction.description);
                  if (description) {
                    return (
                      <div
                        onClick={() => handleActionClick(manager, currAction)}
                      ...
                      >
                        ...
                        <span className={"action-user-time"}>
                          {item.date === "Today"
                            ? moment(currAction.created_at).fromNow() === "a few seconds ago"
                              ? "now"
