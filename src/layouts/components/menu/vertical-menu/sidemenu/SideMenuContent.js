...
class SideMenuContent extends React.Component {
  ...
  isIncludes(permissions, userRoles) {
    if (!permissions || !userRoles) return true;
    return !!permissions.find((permission) => userRoles.indexOf(permission) !== -1);
  }

  userAccess(item) {
    if (!item.userAccess) return true;
    return item.userAccess(this.props.userProfile);
  }
    ...
    const menuItems = navigationConfig
      .filter((item) => {
        if (
          ...
          !this.isIncludes(item.permissions, this.props.userProfile.roles) &&
          !this.userAccess(item)
        ) {
          return this.redirectUnauthorized();
        }
        return this.isIncludes(item.permissions, this.props.userProfile.roles);
      })
      .map((item) => {
        ...
        if (Object.keys(this.props.userProfile).length) {
          if (
            ...
            item.type === "item" &&
            item.navLink === this.props.activePath &&
            (!this.isIncludes(item.permissions, this.props.userProfile.roles) || !this.userAccess(item))
          ) {
            return this.redirectUnauthorized();
          }
        }
