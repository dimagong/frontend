...
export function ObjectFieldTemplate(props) {
  ...
  const checkUiOptionField = (objKey, option) => {
    ...
    if (
      this.state.uiSchema[objKey][Constants.UI_OPTIONS] &&
      this.state.uiSchema[objKey][Constants.UI_OPTIONS][option]
    ) {
      return true;
    }
    return false;
  ...
  const isElementInSection = (elementContentKey, sectionName) => {
    return (
      elementContentKey in this.state.uiSchema.sections &&
      this.state.uiSchema.sections[elementContentKey] === sectionName
    );
  };
    ...
    return Object.keys(groupedElements).map((groupName, index) => {
      ...
      if (this.state.uiSchema.fieldsOrdering && this.state.uiSchema.fieldsOrdering.length) {
        ...
        const filteredOrderingElementNames = this.state.uiSchema.fieldsOrdering.filter(
          (elementName) => elementNames.indexOf(String(elementName)) !== -1
        );
        elementContent = filteredOrderingElementNames.map((nextName) => {
          return elementContent.find(
            (elementContentElement) => String(elementContentElement.name) === String(nextName)
          );
        });
      }
      // todo end ordering
      ...
      elementContent = elementContent.map((element) => {
        if (isElementInSection(element.name, sectionName)) {
          const isElementHidden = (elementKey) => {
            let isHidden =
              elementKey in this.state.uiSchema &&
              Constants.UI_HIDDEN in this.state.uiSchema[elementKey] &&
              this.state.uiSchema[elementKey][Constants.UI_HIDDEN]
                ? { display: "none" }
                : {};
          ...
          return <Field {...fieldProps}>{renderCustomFieldsComponents(element)}</Field>;
        }
        return null;
      });
      ...
      let isHidden =
        groupName in this.state.uiSchema.groupStates &&
        Constants.UI_HIDDEN in this.state.uiSchema.groupStates[groupName] &&
        this.state.uiSchema.groupStates[groupName][Constants.UI_HIDDEN]
          ? { display: "none" }
          : {};
      ...
      return <Groups {...propsGroup}>{elementContent}</Groups>;
    });
  };
    ...
    return Object.keys(groupedElements).map((groupName, index) => {
      ...
      let elementContent = groupedElements[groupName].map((element) => {
        if (isElementInSection(element.name, sectionName)) {
          ...
          return <Field {...fieldProps}>{element.content}</Field>;
        }
        return null;
      });
  ...
  const isSectionHidden = (section, effect) => {
    let isHidden =
      section in this.state.uiSchema.sectionStates &&
      Constants.UI_HIDDEN in this.state.uiSchema.sectionStates[section] &&
      this.state.uiSchema.sectionStates[section][Constants.UI_HIDDEN]
        ? { display: "none" }
        : {};
  ...
  const isSectionDisabled = (section) => {
    return section in this.state.uiSchema.sectionStates &&
      Constants.UI_DISABLED in this.state.uiSchema.sectionStates[section] &&
      this.state.uiSchema.sectionStates[section][Constants.UI_DISABLED]
      ? { disabled: "disabled" }
      : {};
  };
  const getErrors = () => {
  ...
  const getProgress = (section) => {
    let isFieldEmpty = (data) =>
      data === "" || (Array.isArray(data) && data.length === 0) || data === null || data === false;
    ...
    let currentSectionRequiredFields = currentSectionFields.filter((field) =>
      requiredFieldsToString.some((rField) => String(field) === rField)
    );

    currentSectionRequiredFields = currentSectionRequiredFields.filter((field) => {
      if (
        field in this.state.uiSchema &&
        ((Constants.UI_DISABLED in this.state.uiSchema[field] &&
          this.state.uiSchema[field][Constants.UI_DISABLED] &&
          this.state.dFormTemplate.status !== "submitted") ||
