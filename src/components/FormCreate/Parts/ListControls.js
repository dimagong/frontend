...
export function listControls(properties) {
  ...
  const renderConfigFields = (objKey, index) => {
    ...
    const onChangeResourceManagerFieldFile = (resourceManagerFieldId) => {
      this.setState({
        schemaPropertyEdit: { ...this.state.schemaPropertyEdit, resource_manager_field_id: resourceManagerFieldId },
      });
    };
            ...
            <Row>
              ...
              <Col md="12">
                <FormGroup>
                  ...
                  {renderReactSelectColumn("action", [{ value: "default", label: "Compile on dForm association" }])}
                </FormGroup>
              </Col>
              <Col md="12">{labelForControls}</Col>
              <Col md="12">
        ...
        case Constants.FIELD_TYPE_DATE: {
          return (
            <div>
              ...
              <div className="mt-1">{renderRequiredColumn(objKey, "Required?")}</div>
              <div>{renderLabelShowing(objKey)}</div>
            </div>
          );
        }
    ...
    return (
      <div
        className={
          objKey in this.state.uiSchema.columnsClasses ? this.state.uiSchema.columnsClasses[objKey] : "col-md-12"
        }
        key={"control-edit-" + objKey + "-" + index}
      >
        {this.state.schema.properties[objKey].title
          ? renderLabel("property-" + objKey, this.state.schema.properties[objKey].title)
          : "Empty title"}
        <div className="form-group position-relative">
          ...
          <div className="d-flex dform-input-setting">
            <div className="vertical-center dform-input">
              <ElementEditModal
                onOpen={() => this.elementEditModalOpened(objKey)}
              ...
              >
                <div className="form-group">
                  ...
                  >
                    {this.state.controlTypes.map((type, indexType) => (
                      <option key={indexType}>{type}</option>
                    ))}
                  </select>
                </div>
            ...
            <div className="vertical-center dform-input mr-0">
              <DependencyEditModal
                onOpen={this.uiSettingsOpen.bind(this, objKey)}
                onSave={this.uiSettingsSave.bind(this, "fields", objKey)}
                onDelete={() => {
              ...
              >
                <div className="">
                  <div className="col-md-12 form-group">
                    ...
                    >
                      {Object.keys(this.state.uiSchema.onlySections).map((menuName, index) => (
                        <option key={index}>{menuName}</option>
                      ))}
                    </select>
                  </div>
                  <div className="col-md-12 form-group">
                  ...
                  <div className="col-md-12 form-group">
                    ...
                    >
                      {["", "col-md-6", "col-md-12"].map((col, index) => (
        ...
        return Object.keys(groupedElements[groupName]).map((key) => {
          ...
          return (
            <Card key={groupName}>
              ...
              <CardBody>{renderConfigFields(key, index)}</CardBody>
            </Card>
          );
        });
      }
    ...
    return Object.keys(this.state.uiSchema.sectionGroups).map((groupName, index) => {
      if (groupedElementsKeys.indexOf(groupName) !== -1) {
        ...
        if (this.state.uiSchema.fieldsOrdering && this.state.uiSchema.fieldsOrdering.length) {
          ...
          elementContent = this.state.uiSchema.fieldsOrdering.filter(
            (elementName) => stringProps.indexOf(String(elementName)) !== -1
          );
        }
        // todo end ordering
          ...
          <div className="border px-2" key={sectionName + "_" + groupName}>
            <div className="title pt-2 pb-0">
              <span className="text-bold-500 font-medium-2 ml-50">
                {groupName}
                <div className="float-right">{this.modalEditDependencies("groups", groupName)}</div>
              </span>
              <hr />
            </div>
      ...
      return (
        <div className="border px-2 form-create__group-min-height " key={sectionName + "_" + groupName}>
          <div className="title pt-2 pb-0">
            <span className="text-bold-500 font-medium-2 ml-50">
              {groupName}
              <div className="float-right">{this.modalEditDependencies("groups", groupName)}</div>
  ...
  const renderObject = () => {
    return (
      <div>
        <Row>{renderElementsWithoutGroupsAndSections()}</Row>
          ...
          {onlySections.map((section, index) => (
            <NavItem key={section}>
              ...
              >
                ...
                <div className="ml-1 float-right">{this.modalEditDependencies("sections", section)}</div>
              </NavLink>
            </NavItem>
          ))}
