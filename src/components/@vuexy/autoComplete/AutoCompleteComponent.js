...
class Autocomplete extends React.Component {
    ...
    if (this.state.userInput !== "" && this.state.userInput.length > 2) {
      ...
      if (!repeats.length) {
        localStorage.setItem(
          "recentSearches",
          JSON.stringify([this.state.userInput, ...this.state.recentSearches.slice(0, 2)])
        );
        this.setState({
  ...
  renderSuggestion = (suggestion, filterKey, filteredData, activeSuggestion) => (
    <li
      ...
      onClick={(e) => this.onSuggestionItemClick(suggestion.link ? suggestion.link : null, e)}
  ...
  renderSuggestionsGroups = (suggestionsGroup) => {
    ...
    return (
      <React.Fragment key={suggestionsGroup[filterHeaderKey]}>
        ...
        {suggestionsGroup.map((suggestion) => this.renderSingleSuggestionsGroup(suggestion.data))}
      </React.Fragment>
    );
  };
  ...
  renderSuggestions = () => {
    ...
    if (grouped === undefined || grouped === null || !grouped) {
      ...
      return suggestions.map((suggestion) => this.renderSuggestionsGroups(suggestion));
    }
  };
  ...
  render() {
          ...
          {!!this.state.recentSearches.length && (
            <div className="m-1 border-top-secondary ">
              ...
              {this.state.recentSearches.map((rs) => (
                <div className="h5 cursor-pointer" onClick={() => this.setState({ userInput: rs })}>
                  {rs}
                </div>
              ))}
            </div>
    ...
    return (
      <div className="vx-autocomplete-container" ref={this.containerRef}>
        <input
          ref={this.wrapperRef}
          type="text"
          onChange={(e) => {
