...
const FilterOptions = ({ filter, currFilterOption, options, handleFilterOptions }) => {
    ...
    <span className={'filter-right'}>
        <span>
          <span className={'filter-check'} onClick={() => handleFilterOptions('remove', item)}>
            <svg style={filter.type[currFilterOption] === 'cross' && !filter[currFilterOption].find(el => el === item) ? {fill: '#dc3545', whiteSpace: 'pre', borderRadius: '6px'} : {fill: '#95989a', whiteSpace: 'pre', borderRadius: '6px'}}
          ...
          <span className={'filter-check'} onClick={() => handleFilterOptions('add', item)}>
            <svg style={filter.type[currFilterOption] === 'check' && filter[currFilterOption].find(el => el === item) ? {fill: '#7367f0', whiteSpace: 'pre', borderRadius: '6px'} : {fill: '#95989a', whiteSpace: 'pre', borderRadius: '6px'}}
              xmlns="http://www.w3.org/2000/svg" viewBox="0 0 11 12" width="14" height="14">
