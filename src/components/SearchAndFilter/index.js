...
const SearchAndFilter = (props) => {
  ...
  const onCustomCalendarChange = (value) => {
  ...
  return (
    <div className={"search-and-filter-container"}>
      ...
      {!!appliedFilter?.applications?.length && (
        <div
          className={`modal-filter-tabs ${filterTabPosition === "left" && "left-orientation-tabs"} ${
            filterTabPosition === "right" ? "right-orientation-tabs" : ""
          }`}
        >
          {Object.keys(appliedFilter).map((item) => {
            if (item !== "type" && appliedFilter[item].length > 0) {
              return (
                <Button className={"filter-tab member-firm-filter-tab filter-close-button"} variant={"dark"} key={item}>
                  ...
                  <span onClick={() => clearOneFilterType(item)} className={"close-nav"}>
