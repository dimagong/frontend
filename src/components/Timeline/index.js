      ...
      {indexes.map((i, idx) => {
        ...
        return (
          <span key={idx}>
            ...
            {text.substring(
              i.finish + 4,
              currIndex + 1 === indexes.length ? text.length : indexes[currIndex + 1].start
            )}
          </span>
        );
      })}
...
export const getEditMessage = (editData) => {
  ...
  let changedOptions = optionsData.filter(
    (item) =>
      item.old !== item.new &&
      (item.old || item.new) &&
  ...
  for (let i = 0; i < changedOptions.length; ++i) {
    ...
    newMessage.push(
      <span key={changedOptions[i].type}>
        ...
        >
          <span className={"activity-profile-update"}>
            <strong>{changedOptions[i].name.toLowerCase()}</strong>
          </span>
        </OverlayTrigger>
        {addBreaker}
      </span>
...
const Timeline = ({
  activity,
  loadMoreData,
  noActivitiesMessage = "This manager has no activities yet",
  className,
}) => {
