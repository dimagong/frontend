...
const getActivitiesSuccess = (state, { payload }) => {
  ...
  if (
    state.user.managers[managerIndex].activity &&
    state.user.managers[managerIndex].activity.current_page < payload.response.current_page
  ) {
    let newData = (state.user.managers[managerIndex].activity.data = state.user.managers[
      managerIndex
    ].activity.data.concat(payload.response.data));
...
const updateActivitiesSuccess = (state, { payload }) => {
  ...
  if (!state.user.managers[managerIndex].activity) {
    ...
    let startingIndex = payload.response.data.findIndex(
      (item) => item.id === state.user.managers[managerIndex].activity.data[0].id
    );
...
const getDashboardDataSuccess = (state, { payload }) => {
  ...
  state.isError = null;
  let currDataIndex =
    state?.user?.dashboard?.data?.length > 0
  ...
  if (
    currData?.userDFormActivities &&
    payload.response.userDFormActivities.current_page > currData.userDFormActivities.current_page
  ) {
...
const getDashboardSnapshotDataSuccess = (state, { payload }) => {
  ...
  if (payload.payload.page === 1) {
    currData.userDFormActivities = payload.response.userDFormActivities;
    currData.userDFormActivitiesSchedule = payload.response.userDFormActivitiesSchedule
      ? payload.response.userDFormActivitiesSchedule
      : [];
    currData.key = payload.payload.key;
    if (currDataIndex === -1) {
      state.user.dashboard.data.push(currData);
...
const getDashboardActivitySuccess = (state, { payload }) => {
  ...
  if (
    currData?.usersActivities &&
    payload.response.usersActivities.current_page > currData.usersActivities.current_page
  ) {
...
const getUserByIdSuccess = (state, { payload }) => {
  ...
  state.user.manager = {
    ...payload,
    onboarding: payload.onboardings.find((onboarding) => onboarding.id === state.user.manager.onboarding.id),
  };
...
const deleteUserOnboardingSuccess = (state, { payload }) => {
  ...
  state.user.managers[currentManager].onboardings = state.user.managers[currentManager].onboardings.filter(
    (onboarding) => onboarding.id !== payload.id
  );
};
...
const updateUserOnboardingReviewersSuccess = (state, { payload }) => {
  ...
  const onboardingIndex = state.user.managers[managerIndex].onboardings.findIndex(
    (onboarding) => onboarding.id === payload.onboardingId
  );
...
const removeUserOrganizationSuccess = (state, { payload }) => {
  ...
  user.organizations[payload.response.type] = user.organizations[payload.response.type].filter(
    (org) => org.id !== payload.response.group_id
  );
...
const allowUserAbilitySuccess = (state, { payload }) => {
  ...
  const editedOrg = user.organizations[payload.data.organization_type].filter(
...
const disallowUserAbilitySuccess = (state, { payload }) => {
  ...
  user.organizations[payload.data.organization_type].filter(
    ({ id }) => id === payload.data.organization_id
