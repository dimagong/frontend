...
export const selectApplicationWorkFlows = (state) =>
  state.onboarding.workflow.workflows.filter((workflow) => workflow.context === "application");
export const selectSurveyWorkFlows = (state) =>
  state.onboarding.workflow.workflows.filter((workflow) => workflow.context === "survey");

export const selectApplicationNotifications = (state) =>
  state.onboarding.notification.notifications.filter((notification) => notification.context === "application");
export const selectSurveyNotifications = (state) =>
  state.onboarding.notification.notifications.filter((notification) => notification.context === "survey");
