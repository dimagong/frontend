...
export const selectManagerById = (id) => (state) =>
  state?.app?.user?.managers.filter((manager) => manager.id === id)[0];
...
export const selectUserActivity = (userId) => (state) =>
  state?.app?.user?.managers.find((item) => item.id === userId).activity;
export const selectDashboardData = (state) => state?.app?.user?.dashboard?.data;
export const selectDashboardDataByKey = (key) => (state) =>
  state?.app?.user?.dashboard?.data?.length > 0
...
export const selectAssignedSurveyById = (id) => (state) =>
  state?.app?.selectedManagerAssignedSurveys.filter((survey) => survey.id === id)[0];
export const selectOnboardingSurveyStats = (id) => (state) =>
  state?.app?.onboardingSurveys.filter((os) => os.id === id)[0].stats;
...
export const selectIsUserMasterSchemaHierarchySearchParamsInitial = _.pipe(
  selectUserMasterSchemaHierarchySearchParams,
  isUserMasterSchemaHierarchySearchParamsInitial
);
